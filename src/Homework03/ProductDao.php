...
class ProductDao
{
    ...
    {
		if ($this->checkUnique($product->ean, $product->id))
		{
			$sth = $this->pdo->prepare("
			UPDATE product
			SET
	 ...
	 * If the id is given than we search for unique ean products
     ...
     * @return bool
     */
    private function checkUnique($ean, $id = NULL)
    {
		$query = "SELECT COUNT(1) FROM product WHERE ean = :ean";
		$queryParams = array(
			':ean' => $ean,
		);
